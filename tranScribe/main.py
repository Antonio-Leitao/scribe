...
def describe(docstring):
    ...
    components = {
        "description": f'<div class="section" id="description"><div class="synopse">{description[0]}</div>,{description[1]}</div>'
    }
...
class Scribe:
    ...
    def get_doctree(self, path):
        for root, dirs, files in os.walk(path):
            ...
            doctree["children"].extend(
                [self.get_doctree(os.path.join(root, d)) for d in dirs]
            )
            for f in files:
                ...
                self.document(
                    object, child_list=doctree["children"], root=self._clean_href(root)
                )
            return doctree
    ...
    def document(self, object, child_list, root):
        ...
        try:
            if inspect.ismodule(object):
                return self.docmodule(object, child_list, root)
            if _isclass(object):
                return self.docclass(object, child_list, root)
            if inspect.isroutine(object):
                return self.docroutine(object, child_list, root)
    ...
    def docmodule(self, object, child_list, root):
        ...
        for key, value in inspect.getmembers(object, _isclass) or inspect.getmembers(
            object, inspect.isroutine
        ):
